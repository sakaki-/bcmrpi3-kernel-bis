...
> If you would have changes you'd like to apply to the kernel config used by this project, please submit a PR targeting the [`conform_config.sh`](https://github.com/sakaki-/bcmrpi3-kernel-bis/blob/master/conform_config.sh) script. Changes should target the *end* of the script, and modularization is prefered wherever possible. Only edits which use the bundled convenience functions `set_kernel_config` and `unset_kernel_config` will be considered for merging. Provide a short comment describing the changes, ideally including a link or bug ID.
...
pi64 ~ # wget -c https://github.com/sakaki-/bcmrpi3-kernel-bis/releases/download/4.14.44.20180601/bcmrpi3-kernel-bis-4.14.44.20180601.tar.xz
pi64 ~ # tar -xJf bcmrpi3-kernel-bis-4.14.44.20180601.tar.xz -C /
pi64 ~ # sync && reboot
```
