...
As with its sister project [bcmrpi-kernel](https://github.com/sakaki-bcmrpi3-kernel), the baseline build configuration is the upstream `bcmrpi3_defconfig`, wherein the first 12 hex digits of the tip commit SHA1 hash are appended to `CONFIGLOCALVERSION` (with a separating hyphen). However, in this `bcmrpi3-kernel-bis` (as compared to [bcmrpi-kernel](https://github.com/sakaki-bcmrpi3-kernel)) project, `-bis` is additionally appended to `CONFIGLOCALVERSION`, and (more importantly) additional tweaks are *also* applied to the kernel configuration before building, by running the `conformconfig.sh` script.

> If you would have changes you'd like to apply to the kernel config used by this project, please submit a PR targeting the `conformconfig.sh` script. Changes should target the *end* of the script, and modularization is prefered wherever possible. Only edits which use the bundled convenience functions `set_kernel_config` and `unset_kernel_config` will be considered for merging. Provide a short comment describing the changes, ideally including a link or bug ID.
...
The corresponding kernel configuration (derived via `make bcmrpi3_defconfig && conform_config.sh && make olddefconfig`) may be viewed [here](https://github.com/sakaki-/bcmrpi3-kernel-bin/blob/master/config). The 'baseline' `bcmrpi3_defconfig` may be viewed [here](https://github.com/sakaki-/bcmrpi3-kernel-bin/blob/master/bcmrpi3_config), and a diff between the 'tweaked' and 'baseline' configurations may be viewed [here](https://github.com/sakaki-/bcmrpi3-kernel-bin/blob/master/vs_bcmrpi3_config.diff).

> A list of all releases may be seen [here](https://github.com/sakaki-/bcmrpi3-kernel-bin/releases).
...
pi64 ~ # wget -c https://github.com/sakaki-/bcmrpi3-kernel-bin/releases/download/4.14.44.20180601/bcmrpi3-kernel-bin-4.14.44.20180601.tar.xz
pi64 ~ # tar -xJf bcmrpi3-kernel-bin-4.14.44.20180601.tar.xz -C /
pi64 ~ # sync && reboot
```
...
```console
pi64 ~ # emaint sync --repo rpi3
pi64 ~ # emerge -av =bcmrpi3-kernel-bis-bin-4.14.44.20180601
pi64 ~ # reboot
```
